...
alias ogp="vim ~/.oh-my-zsh/plugins/git/git.plugin.zsh"
alias bb="byobu"
...
alias gcs="grp 'Completed superstep '"
alias gfs="grp 'finishSuperstep: Superstep "
...
function grd() {
	if [[ $# -ne 3 ]] ; then
			echo 'grd string-to-search dir-to-search search-into-these-files-name'
			return
	fi
...
if [ `uname` = "Darwin" ] && [ -r ~/bhim/configs/mac.zsh ]; then
. $HOME/bhim/configs/mac.zsh
fi
